;;; shader-mode.el --- major mode for Open GLSL shader files
...
;; Version: 2.4
;; X-URL: https://github.com/jimhourihan/shader-mode
;;
;; Original X-URL http://artis.inrialpes.fr/~Xavier.Decoret/resources/shader-mode/
...
;;
;; Modifications from the 1.0 version of shader-mode (jimhourihan):
...
;; If shader-mode is not part of your distribution, put this file into your
;; load-path and the following into your ~/.emacs:
;;   (autoload 'shader-mode "shader-mode" nil t)
...
;;   (add-to-list 'auto-mode-alist '("\\.geom\\'" . shader-mode))
...
(defvar shader-mode-menu nil "Menu for GLSL mode")

(defvar shader-mode-hook nil "GLSL mode hook")
...
(defvar shader-mode-hook nil)

(defvar shader-mode-map
  (let ((shader-mode-map (make-sparse-keymap)))
    (define-key shader-mode-map [S-iso-lefttab] 'ff-find-other-file)
    shader-mode-map)
...
(defvar shader-mode-syntax-table
  (let ((shader-mode-syntax-table (make-syntax-table)))
    (modify-syntax-entry ?/ ". 124b" shader-mode-syntax-table)
    (modify-syntax-entry ?* ". 23" shader-mode-syntax-table)
    (modify-syntax-entry ?\n "> b" shader-mode-syntax-table)
    (modify-syntax-entry ?_ "w" shader-mode-syntax-table)
    shader-mode-syntax-table)
  "Syntax table for shader-mode.")
...
(easy-menu-define glsl-menu shader-mode-map
  "GLSL Menu"
    `("GLSL"
...
;;;###autoload
(define-derived-mode shader-mode prog-mode "GLSL"
  ...
  (add-to-list 'align-c++-modes 'shader-mode)
  (c-run-mode-hooks 'c-mode-common-hook)
  (run-mode-hooks 'shader-mode-hook)
